...
export default function Inventaire() {
  ...
  const [receptionScan, setReceptionScan] = useState("");
  const [receptionQty, setReceptionQty] = useState("1");
  const [receptionLoading, setReceptionLoading] = useState(false);
  const [receptionSaving, setReceptionSaving] = useState(false);
  const [receptionFoundItem, setReceptionFoundItem] =
    useState<InventaireItem | null>(null);
  const [receptionMatchedBy, setReceptionMatchedBy] =
    useState<"sku" | "supersed" | null>(null);
  const [receptionRows, setReceptionRows] = useState<ReceptionItem[]>([]);
  ...
  const receptionScanRef = useRef<HTMLInputElement | null>(null);
  ...
  useEffect(() => {
    if (!receptionOpen) return;

    const t = window.setTimeout(() => {
      receptionScanRef.current?.focus();
      receptionScanRef.current?.select();
    }, 50);
  ...
  function makeReceptionKey() {
    return `reception-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  }

  function resetReceptionState() {
    setReceptionScan("");
    setReceptionQty("1");
    setReceptionFoundItem(null);
    setReceptionMatchedBy(null);
    setReceptionRows([]);
    setReceptionNote("");
  ...
  async function findReceptionItemByCode(code: string) {
    const q = code.trim();
    ...
    try {
      ...
      if (!error) {
        ...
        if (rows.length > 0) {
          ...
          if (itemData) {
            setReceptionFoundItem(itemData as InventaireItem);
            setReceptionMatchedBy(
      ...
      if ((directData || []).length > 0) {
        setReceptionFoundItem(directData![0] as InventaireItem);
        setReceptionMatchedBy("sku");
        return;
      }
      ...
      if ((supersedData || []).length > 0) {
        const itemId = String((supersedData![0] as any).item_id || "");
        ...
        if (itemData) {
          setReceptionFoundItem(itemData as InventaireItem);
          setReceptionMatchedBy("supersed");
          return;
        }
      }

      alert("Aucune pièce trouvée pour ce code.");
    } catch (e: any) {
      console.error("Erreur recherche réception:", e);
      alert(e?.message || "Erreur lors de la recherche de la pièce.");
    } finally {
      setReceptionLoading(false);
    }
  }

  async function quickScanReception(rawCode: string) {
    ...
    try {
      let foundItem: InventaireItem | null = null;
      let matchedBy: "sku" | "supersed" | null = null;
      ...
      if (!error) {
        ...
        if (rows.length > 0) {
          ...
          if (itemData) {
            foundItem = itemData as InventaireItem;
            matchedBy =
              match.matched_by === "supersed"
                ? "supersed"
                : "sku";
          }
        }
      ...
      if (!foundItem) {
        ...
        if ((directData || []).length > 0) {
          foundItem = directData![0] as InventaireItem;
          matchedBy = "sku";
        }
      }

      if (!foundItem) {
        alert("Aucune pièce trouvée.");
        return;
      }

      setReceptionRows((rows) => {
        const existingIndex = rows.findIndex(
          (row) => row.item_id === foundItem!.id
        );
        ...
        return [
          ...rows,
          {
            key: makeReceptionKey(),
            item_id: foundItem.id,
            sku: foundItem.sku,
            nom: foundItem.nom,
            quantite_actuelle: Number(foundItem.quantite || 0),
            quantite_recue: "1",
      ...
      console.error("Erreur scan réception:", e);
      alert(e?.message || "Erreur scan réception");
    } finally {
      setReceptionLoading(false);
    }
  }
  ...
  function addReceptionRow() {
    if (!receptionFoundItem) {
      alert("Aucune pièce sélectionnée.");
      return;
    }

    const qty = toNumberOrZero(receptionQty);
    ...
    setReceptionRows((rows) => {
      const existingIndex = rows.findIndex((row) => row.item_id === receptionFoundItem.id);

      if (existingIndex >= 0) {
        return rows.map((row, index) => {
          if (index !== existingIndex) return row;

          const current = toNumberOrZero(row.quantite_recue);

          return {
            ...row,
            quantite_recue: String(current + qty),
          };
        });
      }
      ...
      return [
        ...
        {
          key: makeReceptionKey(),
          item_id: receptionFoundItem.id,
          sku: receptionFoundItem.sku,
          nom: receptionFoundItem.nom,
          quantite_actuelle: Number(receptionFoundItem.quantite ?? 0),
          quantite_recue: String(qty),
          matched_by: receptionMatchedBy,
        },
      ];
    });
  ...
  function updateReceptionRow(key: string, quantiteRecue: string) {
    setReceptionRows((rows) =>
      rows.map((row) =>
        row.key === key ? { ...row, quantite_recue: quantiteRecue } : row
      )
    );
  }

  function removeReceptionRow(key: string) {
    setReceptionRows((rows) => rows.filter((row) => row.key !== key));
  }

  async function confirmReception() {
    if (receptionRows.length === 0) {
      alert("Aucune pièce à recevoir.");
      return;
    }
    ...
    try {
      for (const row of receptionRows) {
        const qty = toNumberOrZero(row.quantite_recue);

        if (!Number.isFinite(qty) || qty <= 0) {
          throw new Error(`Quantité invalide pour ${row.sku || row.nom}.`);
        }
        ...
        const currentQty = Number((currentData as any)?.quantite ?? 0);
      ...
      alert(e?.message || "Erreur lors de la réception des pièces.");
      setReceptionSaving(false);
    }
  }
    ...
    <div style={pageWrap}>
      <div style={headerRow}>
        ...
        <div style={headerActions}>
          <button type="button" onClick={openReceptionModal} style={btnSecondary}>
            Réception de pièces
          </button>
          ...
          >
            <div style={modalHeader}>
              <div>
                ...
                <div style={{ opacity: 0.65, marginTop: 4, fontSize: 13 }}>
                  Scanner une pièce. Chaque scan ajoute automatiquement +1 à la réception.
                </div>
              </div>

              <button type="button" style={btnClose} onClick={closeReceptionModal}>
                ×
              </button>
            </div>

            <div style={receptionBody}>
              <div style={receptionCard}>
                <div style={receptionSectionTitle}>Scan / recherche</div>

                <div style={receptionScanRow}>
                  <input
                    ref={receptionScanRef}
                    style={{ ...inputClassic, minWidth: 280, flex: 1 }}
                    value={receptionScan}
                    onChange={(e) => setReceptionScan(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        void quickScanReception(receptionScan);
                      }
                    }}
                    placeholder="Scanner SKU ou supersed"
                    autoComplete="off"
                    spellCheck={false}
                  ...
                  <button
                    type="button"
                    style={btnSecondary}
                    onClick={() => findReceptionItemByCode(receptionScan)}
                    disabled={receptionSaving || receptionLoading}
                  >
                    {receptionLoading ? "Recherche..." : "Rechercher"}
                  </button>
                </div>
                ...
                >
                  Le scanner ajoute automatiquement +1 par scan. Si la pièce existe déjà dans la liste, la quantité augmente automatiquement.
                </div>

                {receptionFoundItem && (
                  <div style={receptionFoundBox}>
                    <div>
                      <div style={{ fontWeight: 900 }}>
                        {receptionFoundItem.sku || "—"} — {receptionFoundItem.nom}
                      </div>
                      <div style={resultsText}>
                        Stock actuel : {fmtQty(receptionFoundItem.quantite)}
                        {receptionMatchedBy === "supersed" ? " • Supersed détecté" : ""}
                      </div>
                    </div>

                    <div style={receptionQtyWrap}>
                      <label style={label}>Qté reçue</label>
                      <input
                        style={{ ...inputClassic, width: 120 }}
                        value={receptionQty}
                        inputMode="decimal"
              ...
              <div style={receptionCard}>
                <div style={receptionSectionTitle}>Réception en cours</div>

                {receptionRows.length === 0 ? (
                  <div style={emptyBox}>Aucune pièce ajoutée à la réception.</div>
                ) : (
                  <div style={{ overflowX: "auto" }}>
                    <table style={table}>
                      <thead>
                        <tr>
                          <th style={th}>SKU</th>
                          <th style={th}>Nom</th>
                          <th style={thRight}>Stock actuel</th>
                          <th style={thRight}>Qté reçue</th>
                          <th style={thRight}>Nouveau stock</th>
                          <th style={thActions}>Actions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {receptionRows.map((row, index) => {
                          const rowBg = index % 2 === 0 ? "#ffffff" : "#f8fafc";
                          const qty = toNumberOrZero(row.quantite_recue);
                          const nextQty = Number(row.quantite_actuelle || 0) + qty;
                            ...
                            <tr key={row.key} style={{ background: rowBg }}>
                              <td style={{ ...td, background: rowBg }}>
                                ...
                                {row.matched_by === "supersed" && (
                                  <div style={miniInfoText}>Supersed</div>
                                )}
                              </td>
                              <td style={{ ...td, background: rowBg }}>{row.nom}</td>
                              <td style={{ ...tdRight, background: rowBg }}>
                                {fmtQty(row.quantite_actuelle)}
                              </td>
                              <td style={{ ...tdRight, background: rowBg }}>
                                <input
                                  style={{
                                  ...
                                  inputMode="decimal"
                                  onChange={(e) =>
                                    updateReceptionRow(row.key, e.target.value)
                                  }
                                  disabled={receptionSaving}
                                />
                              </td>
                              ...
                              <td style={{ ...td, background: rowBg }}>
                                <div style={actionMenuWrap}>
                                  <button
                                    type="button"
                                    style={btnMiniDanger}
                                    onClick={() => removeReceptionRow(row.key)}
                                    disabled={receptionSaving}
                                  >
                                    Retirer
                                  </button>
                ...
                <div style={{ marginTop: 14 }}>
                  <label style={label}>Note de réception</label>
                  <textarea
                    style={{ ...inputClassic, minHeight: 72, resize: "vertical" }}
                    value={receptionNote}
                    onChange={(e) => setReceptionNote(e.target.value)}
                    placeholder="Ex.: Facture fournisseur, commande, inventaire reçu..."
                    disabled={receptionSaving}
                  />
                </div>
              </div>
            ...
            <div style={modalFooter}>
              ...
              <button
                ...
                disabled={receptionSaving || receptionRows.length === 0}
              >
                {receptionSaving ? "Confirmation..." : "Confirmer réception"}
              </button>
            </div>
          </div>
        </div>
...
const receptionModalCard: React.CSSProperties = {
  width: "min(1100px, 100%)",
  maxHeight: "90vh",
  background: "#fff",
  borderRadius: 18,
...
const receptionBody: React.CSSProperties = {
  padding: 18,
  overflowY: "auto",
  display: "grid",
  gap: 14,
...
const receptionCard: React.CSSProperties = {
  border: "1px solid #e2e8f0",
  borderRadius: 14,
  background: "#fff",
  padding: 14,
...
const receptionSectionTitle: React.CSSProperties = {
  fontSize: 15,
  fontWeight: 900,
  color: "#0f172a",
  marginBottom: 12,
...
const receptionScanRow: React.CSSProperties = {
  display: "flex",
  alignItems: "center",
  gap: 10,
  flexWrap: "wrap",
...
const receptionFoundBox: React.CSSProperties = {
  marginTop: 12,
...
const receptionQtyWrap: React.CSSProperties = {
  display: "grid",
  gap: 4,
};
