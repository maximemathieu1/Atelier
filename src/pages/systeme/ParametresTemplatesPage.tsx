...
export default function ParametresTemplatesPage() {
  ...
  const [menuType, setMenuType] = useState<"template" | "template_item" | null>(null);
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      ...
      if (!target) return;
      const inMenu = target.closest('[data-menu-root="param-template"]');
      if (!inMenu) {
        setMenuOpenId(null);
        setMenuType(null);
      }
  ...
  async function deleteTemplate(row: EntretienTemplateRow) {
    if (busy) return;
    setMenuOpenId(null);
    setMenuType(null);
    ...
    setBusy(true);
    try {
      const { error } = await supabase.from("entretien_templates").delete().eq("id", row.id);
      if (error) throw error;
      await loadAll();
  ...
  async function toggleTemplateItemActif(row: EntretienTemplateItemRow) {
    if (busy) return;
    setMenuOpenId(null);
    setMenuType(null);
    setBusy(true);
  ...
  async function deleteTemplateItem(row: EntretienTemplateItemRow) {
    if (busy) return;
    setMenuOpenId(null);
    setMenuType(null);
      ...
      <div style={styles.page}>
        <div style={styles.card}>
          <div style={{ ...styles.row, justifyContent: "space-between" }}>
            <div>
              <div style={{ fontSize: 18, fontWeight: 950 }}>Paramètres templates</div>
                  ...
                  filteredTemplates.map((tpl) => {
                    ...
                    return (
                      <tr key={tpl.id}>
                        ...
                        <td style={styles.td}>
                          <div style={styles.menuWrap} data-menu-root="param-template">
                            <button
                              type="button"
                                  ...
                                  {menuOpenId === r.id && menuType === "template_item" && (
                                    <div style={styles.menu}>
                                      ...
                                      <button
                                        type="button"
                                        style={styles.menuItem}
                                        onClick={() => toggleTemplateItemActif(r)}
                                      >
