...
const tabs: { key: FactureStatut | "toutes"; label: string }[] = [
  ...
  { key: "autorisee_paiement", label: "Autorisées" },
  { key: "payee", label: "Payées" },
  { key: "refusee", label: "Refusées" },
  { key: "garantie", label: "Garantie" },
  { key: "retour_credit", label: "Retour / crédit" },
...
function getBtUnite(bt: BtOption) {
  return Array.isArray(bt.unites) ? bt.unites[0] : bt.unites;
}
...
function statutLabel(statut: FactureStatut) {
  ...
  if (statut === "autorisee_paiement") return "Autorisée";
  if (statut === "payee") return "Payée";
  if (statut === "refusee") return "Refusée";
  if (statut === "garantie") return "Garantie";
  if (statut === "retour_credit") return "Retour / crédit";
...
function statutStyle(statut: FactureStatut): CSSProperties {
  ...
  if (statut === "archivee") return { ...styles.badge, ...styles.badgeMuted };
  if (statut === "en_validation") return { ...styles.badge, ...styles.badgeInfo };
  if (statut === "refusee") return { ...styles.badge, ...styles.badgeDanger };
  if (statut === "garantie") return { ...styles.badge, ...styles.badgePurple };
  if (statut === "retour_credit") return { ...styles.badge, ...styles.badgeOrange };
...
export default function FacturesFournisseurs() {
  ...
  async function loadBtOptions() {
    ...
    setBtOptions((data || []) as unknown as BtOption[]);
  }
    ...
    try {
      for await (const [fournisseurName, fournisseurHandle] of handle.entries()) {
        ...
        for await (const [fileName, fileHandle] of fournisseurHandle.entries()) {
         ...
         // 🔴 Vérifie que le fichier est réellement disponible (cloud sync)
if (!file || file.size === 0) {
  console.warn("Fichier non disponible localement :", fileName);
  skipped++;
  continue;
}
...
// 🔴 Si upload échoue → on NE crée PAS la facture
if (uploadError) {
  console.warn("Upload facture échoué :", uploadError.message);
  skipped++;
  continue;
}
...
if (insertError) {
  console.warn("Erreur insert facture :", insertError.message);
  skipped++;
  continue;
}
...
imported++;        }
      ...
      setMessage(`${imported} facture(s) importée(s). ${skipped} fichier(s) ignoré(s). Regarde la console si 0 import.`);
      await loadFactures();
    } catch (err: any) {
  ...
  async function setStatut(facture: FactureFournisseur, statut: FactureStatut) {
    ...
    if (statut === "payee") {
      update.payee_par = userData.user?.id || null;
      update.payee_email = userData.user?.email || null;
    ...
    if (statut === "refusee") {
      update.refuse_par = userData.user?.id || null;
      update.refuse_email = userData.user?.email || null;
      update.refuse_le = new Date().toISOString();
    }

    if (statut === "garantie") {
      update.type_traitement = "garantie";
      update.traitement_mode = "garantie";
    }

    if (statut === "retour_credit") {
      update.type_traitement = "retour_credit";
      update.traitement_mode = "retour_credit";
    }
        ...
        <div style={styles.rightPanel}>
          ...
          {!selected ? (
            ...
            <>
              ...
              <div style={styles.formGroup}>
                <label style={styles.label}>Note interne / annotation</label>
                <textarea
                  style={styles.textarea}
                  value={note}
                  onChange={(e) => setNote(e.target.value)}
                  placeholder="Note pour comptabilité, atelier, garantie, retour, refus..."
                />
              </div>
              ...
              <div style={styles.actionsStack}>
                ...
                <button
                  type="button"
                  style={styles.purpleFull}
                  onClick={() => selected && setStatut(selected, "garantie")}
                  disabled={processing}
                >
                  Classer en garantie
                </button>
                ...
                <button
                  type="button"
                  style={styles.orangeFull}
                  onClick={() => selected && setStatut(selected, "retour_credit")}
                  disabled={processing}
                >
                  Retour / crédit demandé
                </button>

                <button
                  type="button"
                  style={styles.dangerFull}
                  onClick={() => selected && setStatut(selected, "refusee")}
                  disabled={processing}
                >
                  Refuser
                </button>
                ...
                <button
                  type="button"
                  style={styles.secondaryFull}
                  onClick={() => selected && setStatut(selected, "payee")}
                  disabled={processing}
                >
                  Marquer payée
                </button>
                ...
                <button
                  type="button"
                  style={styles.secondaryFull}
                  onClick={() => selected && setStatut(selected, "archivee")}
                  disabled={processing}
                >
                  Archiver
                </button>
